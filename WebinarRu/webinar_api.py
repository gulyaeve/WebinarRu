...
class WebinarAPI(BaseAPI):
    ...
    async def get_events_for_user(
        ...
        @param access_settings: доступ к мероприятиям. (isPasswordRequired — доступ с паролем; isRegistrationRequired — доступ с регистрацией; isModerationRequired — доступ с залом ожидания)
        @param access: уровень доступа к мероприятиям (1 - свободный доступ; 3 - свободный доступ с паролем; 4 - регистрация; 6 - регистрация с паролем; 8 - регистрация с последующей ручной модерацией участников; 10 - регистрация с последующей ручной модерацией участников и с паролем)
    ...
    async def delete_event_session(
        ...
        @param send_email: рассылка писем с платформы Webinar.ru.  Флаг определяет отправку письма "К сожалению, мероприятие отменено"
        @param event_session_id: идентификатор вебинара (eventsessionID)
        @return: True если удаление успешно
        """
        params = {}
    ...
    async def create_event(
        ...
        @param access_settings: доступ к мероприятиям (isPasswordRequired — доступ с паролем; isRegistrationRequired — доступ с регистрацией; isModerationRequired — доступ с залом ожидания)
        @param access: уровень доступа к мероприятию (1 - свободный доступ; 3 - свободный доступ с паролем; 4 - регистрация; 6 - регистрация с паролем; 8 - регистрация с последующей ручной модерацией участников; 10 - регистрация с последующей ручной модерацией участников и с паролем)
        ...
        @param rule: [Обязательно для серии] правило генерации дат повторения событий в серийном мероприятии. https://tools.ietf.org/html/rfc5545#section-3.3.10
        ...
        @param lector_ids: ведущий на лендинге. Добавляет иконку с фото и данными ведущего. Передается как массив userID сотрудников организации
        @param tags: теги мероприятия. Массив тегов, которые будут добавлены к мероприятию
        @param duration: длительность мероприятия. Меняет значение "Продолжительность" на лендинге, но не определяет фактическое время завершения. Значение данного поля должно подпадать под регулярное выражение
    ...
    async def edit_event(
        ...
        @param access_settings: доступ к мероприятиям (isPasswordRequired — доступ с паролем; isRegistrationRequired — доступ с регистрацией; isModerationRequired — доступ с залом ожидания)
        @param access: уровень доступа к мероприятию (1 - свободный доступ; 3 - свободный доступ с паролем; 4 - регистрация; 6 - регистрация с паролем; 8 - регистрация с последующей ручной модерацией участников; 10 - регистрация с последующей ручной модерацией участников и с паролем)
        @param status: статус мероприятия (массив). Передав значение STOP, можно завершить мероприятиятие
        @param password: пароль для входа на мероприятие
        @param description: описание мероприятия. Текстовое поле. Отсутствует разметка или верстка
        @param lang: язык интерфейса мероприятия
        @param url_alias: замена названия вебинара в ссылке. Заменяет eventID в ссылке на вебинар
    ...
    async def create_event_session(
        ...
        @param access_settings: доступ к мероприятиям (isPasswordRequired — доступ с паролем; isRegistrationRequired — доступ с регистрацией; isModerationRequired — доступ с залом ожидания)
        @param access: уровень доступа к мероприятию (1 - свободный доступ; 3 - свободный доступ с паролем; 4 - регистрация; 6 - регистрация с паролем; 8 - регистрация с последующей ручной модерацией участников; 10 - регистрация с последующей ручной модерацией участников и с паролем)
        @param start_type: тип запуска мероприятия. Значения: manual — ручной запуск мероприятия; autostart — автоматический запуск мероприятия в указанное время; autowebinar —  автовебинар.
        @param description: описание мероприятия. Текстовое поле.
        @param lang: язык интерфейса мероприятия
        @param starts_at: дата/время начала мероприятия
        @param timezone: часовой пояс вебинара. Передаётся в виде id.
        @param image: фон вебинара. ID файла в файловой системе, который будет использован в качестве фона
        @return: eventsessionId — идентификатор вебинара. Используется для регистрации на вебинары, работы с записью; link — ссылка на сессию. В пользовательских сценариях не используется.
        """
        data = {}
        data.update({"name": name}) if name is not None else ...
        data.update({"accessSettings": access_settings.to_dict}) if access_settings is not None else ...
    ...
    async def edit_event_session(
        ...
        @param send_email: рассылка писем с платформы Webinar.ru. Флаг определяет отправку письма "Вебинар перенесен на другое время"
        @param update_context: параметр, который указывает сервису, что рассылку писем требуется убрать/включить в рамках серийного мероприятия для конкретного события в определенную дату
        @return True если изменено
        """
        data = {}
        data.update({"name": name}) if name is not None else ...
